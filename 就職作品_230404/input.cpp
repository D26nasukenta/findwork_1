...
static DWORD	padReleased[GAMEPADMAX];
...
void UpdatePad(void)
{
	...
	for (i = 0; i < padCount; i++)
	{
		...
		//現在が0で前回が1だと現在が否定されるので1 & 1になる
		padReleased[i] = (  padState[i] ^ lastPadState ) & (~padState[i]);
...
{
	return (button & padReleased[padNo]);
}


float GetLeftStickAngle(int controllerIndex)
{
	XINPUT_STATE state;
	...
	// 角度を計算
	float angle = atan2f(static_cast<float>(stickY), static_cast<float>(stickX));
	//angle = angle * (180.0f / 3.1415926535f); // ラジアンから度に変換
